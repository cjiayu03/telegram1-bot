...
bot.on('callback_query', async (query) => {
  ...
  if (data.startsWith('comment:')) {
    ...
    if (!report) return bot.answerCallbackQuery(query.id, { text: 'Not found' });

    pendingReply[userId] = { incidentId, originMsgId: msgId };

    const prompt = await bot.sendMessage(chatId,
      `Type your comment for incident ${incidentId}`,
      { reply_to_message_id: msgId }
    );

    pendingReply[userId].promptMsgId = prompt.message_id;
    return bot.answerCallbackQuery(query.id, { text: 'Reply enabled' });
  }
...
bot.on('message', async (msg) => {
  ...
  if (pendingReply[userId] && !text.startsWith('/')) {
    const { incidentId } = pendingReply[userId];
...
.header{display:flex;justify-content:space-between;padding:12px 20px;background:#111827}
...
function load(){
  fetch('/api/reports').then(r=>r.json()).then(d=>{
    ...
    document.getElementById('list').innerHTML = d.map(r=>`
      <div class="card" onclick="show(${r.id})">
